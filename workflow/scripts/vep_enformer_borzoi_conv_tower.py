...
class VEPModel(torch.nn.Module):
    ...
    def get_embed(self, x):
        B = len(x)
        embed = self.model(x)
        if isinstance(embed, tuple):  # in the case of Borzoi
            embed = embed[0]
        embed = embed.reshape(B, -1)
        return embed

    def get_scores(self, x_ref, x_alt):
        embed_ref = self.get_embed(x_ref)
        embed_alt = self.get_embed(x_alt)
        return torch.cat((
...
if __name__ == "__main__":
    ...
    columns = ["euclidean_distance", "inner_product", "cosine_distance"]
    window_size = 512
    model = VEPModel(model.model.embedding.conv_tower)
